...
int main(int argc, char** argv) {
	  ...
	  // Connect to a compute device
	    ...
	    // Create the input and output arrays in device memory for our calculation
	     ...
	     if (err != CL_SUCCESS) {
	      ...
	      err = 0;
	      err |= clSetKernelArg(kernel_mean, 0, sizeof(cl_mem), &d_data);
	      err |= clSetKernelArg(kernel_mean, 1, sizeof(cl_mem), &d_mean);
	      err |= clSetKernelArg(kernel_mean, 2, sizeof(cl_mem), &d_covar);
	      ...
	      err = clEnqueueNDRangeKernel(commands, kernel_mean, 1, NULL,
	                                   (size_t*)&global_mean, (size_t*)&local_mean, 0, NULL, &mean_event);
	...
	clGetEventProfilingInfo(mean_event, CL_PROFILING_COMMAND_START, sizeof(time_start), &time_start, NULL);
	clGetEventProfilingInfo(mean_event, CL_PROFILING_COMMAND_END,	sizeof(time_end), &time_end, NULL);
	total_time = time_end - time_start;
	printf("\nExecution time for covar kernel in milliseconds = %0.3f ms\n", (total_time / 1000000.0));
