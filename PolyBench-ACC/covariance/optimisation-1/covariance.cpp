    ...
    for(int i = 0; i < m; i++) {
	 ...
	 printf("INFO: CL_PLATFORM_NAME %s\n",cl_platform_name);
	  ...
	  // Connect to a compute device
	    ...
	    // Create the input and output arrays in device memory for our calculation
	     ...
	     if (err != CL_SUCCESS) {
	         ...
	         return EXIT_FAILURE;
		      ...
		      err = 0;
		      err |= clSetKernelArg(kernel_covar, 0, sizeof(cl_mem), &d_data);
		      err |= clSetKernelArg(kernel_covar, 1, sizeof(cl_mem), &d_covar);
		      ...
		      cl_event covar_event;
		      local_cover[0]  = 1;
		      global_cover[0] = 1;;//ceil(m/(float)local_cover[0])*local_cover[0];
		      err = clEnqueueNDRangeKernel(commands, kernel_covar, 1, NULL,
		                                   (size_t*)&global_cover, (size_t*)&local_cover, 0, NULL, &covar_event);
		...
		clGetEventProfilingInfo(covar_event, CL_PROFILING_COMMAND_START,
			sizeof(time_start), &time_start, NULL);
		clGetEventProfilingInfo(covar_event, CL_PROFILING_COMMAND_END,
